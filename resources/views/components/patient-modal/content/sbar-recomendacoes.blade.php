                ...
                <div class="border-b border-gray-200 mb-4">
                    <nav class="flex space-x-1 overflow-x-auto pb-2">
                        <button @click="activeCpoeCategory = 'cpoe-exames'"
                                ...
                                class="flex-shrink-0 px-3 py-1.5 text-sm font-medium rounded border-b-2 whitespace-nowrap transition-colors">
                ...
                <div>
                    ...
                    <div x-show="activeCpoeCategory === 'cpoe-exames'">
                        @if($patientDetails && isset($patientDetails->cpoe_procedures) && $patientDetails->cpoe_procedures['total_count'] > 0)
                            ...
                            <!-- Kanban por Turnos - Compact Container -->
                            <div class="grid grid-cols-1 lg:grid-cols-3 gap-3 h-80">
                                @foreach(['MANHÃ', 'TARDE', 'NOITE'] as $shift)
                                    @php
                                        $shiftData = $patientDetails->cpoe_procedures['shifts'][$shift] ?? ['count' => 0, 'procedures' => []];
                                    @endphp
                                    
                                    <div class="flex flex-col bg-white border border-gray-200 rounded h-full">
                                        <!-- Shift Header - Compact -->
                                        <div class="flex-shrink-0 px-3 py-2 border-b border-gray-200 bg-gray-50">
                                            <div class="flex items-center justify-between">
                                                <h6 class="font-medium text-gray-800 text-xs uppercase tracking-wide">{{ $shift }}</h6>
                                                <span class="text-xs bg-gray-200 text-gray-700 px-1.5 py-0.5 rounded font-medium">
                                                    {{ $shiftData['count'] }}
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <div class="flex-1 p-2 overflow-y-auto custom-scroll">
                                            @if($shiftData['count'] > 0)
                                                <div class="space-y-1.5">
                                                    @foreach($shiftData['procedures'] as $procedure)
                                                        <div class="bg-gray-50 rounded border p-2 hover:bg-gray-100 transition-colors">
                                                            <div class="text-xs font-medium text-gray-800 mb-1 leading-tight">
                                                                {{ $procedure['procedimento'] }}
                                                            </div>
                                                            <div class="flex items-center justify-between text-xs text-gray-600">
                                                                <span class="font-mono">{{ $procedure['horario'] }}</span>
                                                                ...
                                                                @else
                                                                    <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-amber-100 text-amber-700">
                                                                        <span class="w-1 h-1 bg-amber-500 rounded-full mr-1"></span>
                                                                        Pend.
                                                                    </span>
                                                                @endif
                                                            </div>
                                            ...
                                            @else
                                                <div class="flex flex-col items-center justify-center h-full text-center">
                                                    ...
                                                    <div class="text-gray-500 text-xs">Nenhum procedimento</div>
                                                </div>
                                            @endif
                                        </div>
                                    </div>
