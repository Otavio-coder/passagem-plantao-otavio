<div
    x-data="{
        ...
        get filteredBeds() {
            ...
            if (!q) return this.$wire.beds || [];
            return (this.$wire.beds || []).filter(b =>
...
>
    ...
    >
        ...
        {{-- ─── Panel ───────────────────────────────────────────────────── --}}
        <div
            class="relative bg-white flex flex-col overflow-hidden w-full h-full sm:w-[95vw] sm:h-[92vh] sm:rounded-2xl lg:w-[780px] lg:h-[90vh] shadow-2xl"
            @click.stop
            x-show="show"
            x-transition:enter="transition ease-out duration-250"
        ...
        >
            ...
            <div class="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-[#004D9D] to-[#0071B9] flex-shrink-0">
                <div class="flex items-center gap-2.5 min-w-0">
                    ...
                    @if($totalMessages > 0)
                        <span class="px-2.5 py-0.5 bg-white text-[#004D9D] text-xs font-bold rounded-full flex-shrink-0">{{ $totalMessages }} msgs</span>
                    @endif
                </div>
            ...
            <div class="flex-shrink-0 border-b border-gray-200 bg-white">
                ...
                <div class="flex">
                    <button
                        wire:click="switchShift('previous')"
                        class="flex-1 py-2.5 text-sm font-medium transition-colors border-b-2 {{ $activeShift === 'previous' ? 'border-[#004D9D] text-[#004D9D] bg-blue-50/50' : 'border-transparent text-gray-500 hover:text-gray-700' }}"
                    >
                        Turno Anterior
                        @if($previousShiftLabel)
                            <span class="ml-1 text-xs font-normal opacity-60">{{ $previousShiftLabel }}</span>
                        @endif
                    </button>
                    <button
                        wire:click="switchShift('current')"
                        class="flex-1 py-2.5 text-sm font-medium transition-colors border-b-2 {{ $activeShift === 'current' ? 'border-[#004D9D] text-[#004D9D] bg-blue-50/50' : 'border-transparent text-gray-500 hover:text-gray-700' }}"
                    >
                        Turno Atual
                        @if($currentShiftLabel)
                            <span class="ml-1 text-xs font-normal opacity-60">{{ $currentShiftLabel }}</span>
                        @endif
                    </button>
                </div>
            ...
            <div class="flex-1 overflow-y-auto min-h-0">
                ...
                @elseif(empty($beds))
                    <div class="flex flex-col items-center justify-center h-full text-center px-6 py-16">
                        ...
                        <p class="text-sm text-gray-400 mt-1">Nenhum leito encontrado para este setor.</p>
                    </div>
                ...
                @else
                    ...
                    <div x-show="filteredBeds.length > 0">
                            ...
                            <div class="border-b border-gray-200">
                                ...
                                <div class="px-4 py-3 flex items-center gap-3 sticky top-0 z-10"
                                        ...
                                        <template x-if="bed.status !== 'empty'">
                                            <div>
                                                <div class="flex items-center gap-1.5 flex-wrap">
                                                    <span class="font-semibold text-gray-900 text-base leading-tight" x-text="bed.nome_paciente"></span>
                                    ...
                                    <div class="flex flex-col items-end gap-1 flex-shrink-0">
                                        ...
                                        {{-- Message count (only if has messages) --}}
                                        <template x-if="bed.total_mensagens > 0">
                                            ...
                                            <div class="rounded-xl overflow-hidden"
                                                ...
                                                <div class="flex items-center gap-2 px-3 py-2">
                                                    <template x-if="msg.photo">
                                                        <img :src="'data:image/jpeg;base64,' + msg.photo"
                                                             :alt="msg.user_name"
                                                    ...
                                                    <span class="text-sm font-semibold text-gray-500 tabular-nums flex-shrink-0" x-text="msg.time"></span>
                                                    <template x-if="msg.is_pinned">
                                                        <i class="fas fa-star fa-xs text-amber-400 flex-shrink-0"></i>
                                                    </template>
                                                </div>
                                                {{-- Content --}}
                                                <div class="px-3 pb-2 text-base text-gray-800 leading-relaxed" x-html="msg.content"></div>
            ...
            @if(!$loading && !empty($beds))
                <div class="px-4 py-2.5 border-t border-gray-200 bg-gray-50 flex-shrink-0 flex items-center justify-between text-xs text-gray-500">
                    ...
                    <span class="font-medium">{{ $totalMessages }} {{ $totalMessages === 1 ? 'mensagem' : 'mensagens' }}</span>
                </div>
            @endif
