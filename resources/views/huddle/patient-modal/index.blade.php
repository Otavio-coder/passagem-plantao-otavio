<div
    ...
    x-show="showModal"
    x-cloak
    class="fixed inset-0 z-[100] flex items-start justify-center overflow-y-auto bg-black/50 p-2 sm:p-4"
...
>
    ...
    <div class="relative w-full max-w-3xl bg-white rounded-2xl shadow-2xl my-4 font-montserrat"
         @click.outside="$wire.closeModal()">

        {{-- ── Cabeçalho: navegação + identificação ───────────────────────── --}}
        <div class="rounded-t-2xl bg-[#004D9D] text-white px-4 py-3">
            <div class="flex items-center justify-between gap-3">
                <button wire:click="goToPreviousPatient"
                        @disabled(! $canGoPrevious)
                        class="p-2 rounded-lg hover:bg-white/20 disabled:opacity-30 disabled:cursor-not-allowed"
                ...
                <div class="flex-1 text-center min-w-0">
                    <div class="flex items-center justify-center gap-2">
                        ...
                        @if($mews !== null)
                            <span class="bg-white/15 border border-white/30 text-white text-xs font-bold px-2 py-0.5 rounded-full">
                                MEWS: {{ $mews }}
                            </span>
                        @endif
                    </div>
                ...
                <button wire:click="goToNextPatient"
                        @disabled(! $canGoNext)
                        class="p-2 rounded-lg hover:bg-white/20 disabled:opacity-30 disabled:cursor-not-allowed"
        ...
        {{-- ── Dados do paciente (replica o card SBAR) ─────────────────────── --}}
        <div class="px-4 py-3 border-b border-gray-100 grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-1 text-xs text-gray-700">
            ...
            <div><span class="text-gray-400">Idade:</span> <strong>{{ $p['age'] ?? '—' }}</strong></div>
        ...
        <div class="max-h-[60vh] overflow-y-auto px-4 py-4 space-y-5">

            {{-- ── Gate 72h ────────────────────────────────────────────────── --}}
            <div class="rounded-xl border border-gray-200 p-3">
                <p class="text-sm font-semibold text-gray-800">Paciente com previsão de alta nas próximas 72 horas?</p>
                <p class="text-[11px] text-gray-500 mb-2">Previsão de alta do Tasy: <strong>{{ $tasyPrevAlta ?? 'não registrada' }}</strong></p>
                @can('conduzir huddle')
                    <div class="flex gap-2">
                        <button wire:click="setTriage('huddle')"
                                class="px-3 py-1.5 rounded-lg text-sm font-medium border transition
                                {{ $triageStatus === 'huddle' ? 'bg-green-500 text-white border-green-500' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50' }}">
                            Sim — incluir no Huddle
                        </button>
                        <button wire:click="setTriage('round')"
                                class="px-3 py-1.5 rounded-lg text-sm font-medium border transition
                                {{ $triageStatus === 'round' ? 'bg-gray-600 text-white border-gray-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50' }}">
                            Não — discutir em Round
                        </button>
                    </div>
                @else
                    <p class="text-sm font-medium">{{ $triageStatus === 'round' ? 'Discutir em Round' : ($triageStatus === 'huddle' ? 'Incluído no Huddle' : '—') }}</p>
                @endcan
            </div>

            {{-- ── Checklist (apenas quando incluído no Huddle) ────────────── --}}
            @if($triageStatus === 'round')
                <div class="rounded-xl bg-gray-50 border border-gray-200 p-3 text-sm text-gray-600">
                    Paciente marcado para <strong>Round</strong>. O checklist de alta não se aplica.
                </div>
            @else
                <div class="space-y-3">
                            ...
                            @can('conduzir huddle')
                                <div class="mt-2 flex gap-2">
                                    <button wire:click="answerItem('{{ $code }}', 'sim')"
                                            class="px-3 py-1 rounded-lg text-xs font-medium border {{ $answer === 'sim' ? 'bg-[#004D9D] text-white border-[#004D9D]' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50' }}">
                                        Sim
                                    </button>
                                    <button wire:click="answerItem('{{ $code }}', 'nao')"
                                            class="px-3 py-1 rounded-lg text-xs font-medium border {{ $answer === 'nao' ? 'bg-[#004D9D] text-white border-[#004D9D]' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50' }}">
                                        Não
                                    </button>
                                </div>
                                ...
                                @if($signal === 'red')
                                    ...
                                    @if($item->requiresFollowUp())
                                        <div class="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
                                            <input type="text" wire:model="checklist.{{ $code }}.responsible"
                                            ...
                                            <button wire:click="saveItemDetails('{{ $code }}')"
                                                    class="sm:col-span-2 justify-self-start text-xs px-3 py-1 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700">
                                                Salvar responsável/prazo
                                            </button>
                                        </div>
                                    @endif
                                @endif
                ...
                {{-- ── DPA (previsão de alta registrada no Huddle) ─────────── --}}
                <div class="rounded-xl border border-gray-200 p-3">
                    ...
                    @can('conduzir huddle')
                        <div class="flex flex-wrap items-end gap-2">
                            ...
                            <button wire:click="saveDischarge"
                                    class="text-sm px-3 py-1.5 rounded-lg bg-[#004D9D] text-white hover:bg-[#003a78]">
                                Salvar previsão
                            </button>
                        </div>
        ...
        {{-- ── Rodapé ──────────────────────────────────────────────────────── --}}
        <div class="rounded-b-2xl border-t border-gray-100 px-4 py-3 flex justify-end">
