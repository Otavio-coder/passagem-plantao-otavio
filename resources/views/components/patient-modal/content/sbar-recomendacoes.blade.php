...
<div class="p-3 sm:p-4 lg:p-6 h-full overflow-y-auto">
    ...
    @elseif($currentPatient && (isset($currentPatient['has_patient']) && !$currentPatient['has_patient']))
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
                ...
                <div class="bg-white p-3 rounded-lg border border-gray-200 hover:shadow-md transition-shadow">
                    ...
                    <div class="text-sm text-gray-700 p-2 bg-gray-50 rounded border min-h-[60px]">
                        {{ $patientDetails->prioridade_exames ?? 'Nenhum exame prioritário' }}
                    </div>
                </div>
                ...
                <div class="bg-white p-3 rounded-lg border border-gray-200 hover:shadow-md transition-shadow">
                    ...
                    <div class="text-sm text-gray-700 p-2 bg-gray-50 rounded border min-h-[60px]">
                        {{ $patientDetails->materiais ?? 'Nenhum antimicrobiano' }}
                    </div>
                </div>
                ...
                <div class="bg-white p-3 rounded-lg border border-gray-200 hover:shadow-md transition-shadow">
                    ...
                    <div class="text-sm text-gray-700 p-2 bg-gray-50 rounded border min-h-[60px]">
                        @if($patientDetails && isset($patientDetails->procedimentos_cirurgicos) && is_array($patientDetails->procedimentos_cirurgicos) && count($patientDetails->procedimentos_cirurgicos))
                            @php $firstProc = $patientDetails->procedimentos_cirurgicos[0]; @endphp
                            <div class="font-medium">{{ $firstProc['procedimento'] ?? 'Procedimento' }}</div>
                            <div class="text-xs text-gray-600 mt-1">
                                {{ $firstProc['data_agenda'] ?? 'Data não informada' }}
                                @if(count($patientDetails->procedimentos_cirurgicos) > 1)
                                    <br><span class="italic">+{{ count($patientDetails->procedimentos_cirurgicos) - 1 }} mais</span>
                                @endif
                            </div>
                        @else
