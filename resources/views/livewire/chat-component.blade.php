                    ...
                    <div class="flex items-center justify-between gap-2">
                                ...
                                <div class="history-select-container flex-1 relative">
                                    ...
                                    >
                                        ...
                                        @foreach($groupedSessions as $dateGroup => $sessions)
                                            <optgroup label="📅 {{ $dateGroup }}">
                                                @foreach($sessions as $session)
                                                    <option value="{{ $session['key'] }}">
                                                        &nbsp;&nbsp;{{ $session['shift_label'] }} ({{ $session['messageCount'] }})
                                                    </option>
                                                @endforeach
                                            </optgroup>
                                        @endforeach
                        ...
                        <div class="flex items-center space-x-2 bg-white/15 backdrop-blur-sm rounded-full px-2 py-1 border border-white/20 flex-shrink-0">
                            ...
                            <span class="text-white/90 text-xs font-medium hidden lg:inline">
                                {{ Str::limit($currentUser['name'] ?? 'Usuário', 12) }}
                            </span>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="flex items-start justify-between">
                                <div class="flex items-start flex-1 min-w-0">
                                    <div class="flex-shrink-0 mr-2">
                                        @if(!empty($message['photo']))
                                            <img src="data:image/jpeg;base64,{{ $message['photo'] }}" alt="Foto" class="w-5 h-5 sm:w-6 sm:h-6 rounded-full object-cover" />
                                        @else
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="pinned-content">
                                            <div class="flex items-center space-x-2 mb-1">
                                                <p class="text-xs font-semibold text-gray-800">{{ $message['author'] }}</p>
                                            </div>
                                ...
                                <div class="flex items-center space-x-1 flex-shrink-0 ml-2">
                                    ...
                                    @if(!$viewingHistory && !$isShiftClosed)
                                        ...
                                        >
                                            <span x-show="!isPinning({{ $message['id'] }})">
                ...
                <div class="space-y-1.5 sm:space-y-2">
                    @foreach($regular as $message)
                        <div class="flex {{ $message['is_own'] ? 'justify-end' : 'justify-start' }} group" id="msg-{{ $message['id'] }}">
                            <div class="flex items-start space-x-1.5 sm:space-x-2 {{ $message['is_own'] ? 'flex-row-reverse space-x-reverse' : '' }} max-w-[90%] sm:max-w-[85%]">
                                @if(!empty($message['photo']))
                                    <img src="data:image/jpeg;base64,{{ $message['photo'] }}" alt="Foto" class="w-5 h-5 sm:w-6 sm:h-6 rounded-full object-cover flex-shrink-0" />
                                @else
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="bg-white rounded-lg p-2 sm:p-2.5 shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200 
                                        {{ $message['is_own'] ? 'bg-blue-50 border-blue-200' : '' }}
                                        {{ isset($message['failed']) && $message['failed'] ? 'message-failed' : '' }}
                                        {{ isset($message['is_temporary']) && $message['is_temporary'] ? 'message-sending' : '' }}"
                                    >
                                        <div class="flex items-center space-x-1.5 sm:space-x-2 mb-1">
                                            ...
                                            @if(isset($message['is_temporary']) && $message['is_temporary'])
                                                <span class="ml-1 text-blue-500 text-xs font-medium animate-pulse">Enviando...</span>
                                            @endif
                                            
                                            @if(isset($message['failed']) && $message['failed'])
                                            ...
                                            @if(!$viewingHistory && !$isShiftClosed)
                                                ...
                                                @if($isRealMessage)
                                                    ...
                                                    >
                                                        <span x-show="!isPinning({{ $message['id'] }})">
                                                            @svg('heroicon-o-star', 'w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-400 hover:text-yellow-400')
                                                        </span>
                                                        <span x-show="isPinning({{ $message['id'] }})">
                                                ...
                                                @else
                                                    <div class="ml-1 opacity-50" title="Aguarde o envio">
                                                        @svg('heroicon-o-star', 'w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-300')
                                                    </div>
                                                @endif
                                            @endif
                                        </div>
            ...
            @else
                ...
                <div class="flex flex-col items-center justify-center h-full min-h-[100px] sm:min-h-[120px] p-4">
                    ...
                    <p class="text-gray-400 text-xs mt-0.5 text-center px-2">
                        {{ $viewingHistory ? 'Sem anotações' : 'Inicie a primeira mensagem' }}
                    </p>
                </div>
            @endif
        </div>
                ...
                <form @submit.prevent="sendMessage()">
                    ...
                    <div class="flex justify-between items-center mt-1 sm:mt-1.5">
                        <div class="text-xs text-gray-500">
                            <span id="input-time">{{ now()->format('H:i') }}</span> - {{ Str::limit($currentUser['name'] ?? 'Usuário', 8) }}
                        </div>
        ...
        @else
            ...
            <div class="flex-shrink-0 border-t border-gray-200 bg-gray-100 p-1.5">
                <p class="text-center text-xs text-gray-600 flex items-center justify-center space-x-1">
                    ...
                    <span>{{ $viewingHistory ? 'Histórico' : 'Encerrado' }}</span>
                </p>
            </div>
        @endif
    @endif
