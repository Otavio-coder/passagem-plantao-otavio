<div x-show="activeRecomendacaoTab === 'tab-dial'" style="display:none;" class="pt-3">
    ...
    <div x-show="dial.items.length > 0" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2">
        <template x-for="d in dial.items" :key="String(d.id ?? d.modalidade ?? 'dial') + '-' + String(d.dt_inicio ?? '')">
            <div class="bg-white rounded-md border border-rose-200 px-2 py-2 min-h-[145px] flex flex-col">
                ...
                <div class="mt-1.5 flex flex-wrap gap-x-2 gap-y-1 text-[11px] leading-tight text-gray-600">
                    <span x-show="d.prescriber_id"><span class="font-semibold text-gray-500">ID Presc:</span> <span x-text="d.prescriber_id"></span></span>
                    ...
                    <span x-show="d.sector_name || d.sector_code"><span class="font-semibold text-gray-500">Setor:</span> <span x-text="d.sector_name || ('Setor ' + d.sector_code)"></span></span>
                ...
                <p x-show="d.observacao" class="mt-1 text-[11px] text-gray-600 leading-tight mt-auto">
                    <span class="font-semibold text-gray-500">Observacao:</span> <span x-text="d.observacao"></span>
                </p>
                <p x-show="d.justificativa" class="mt-1 text-[11px] text-gray-600 leading-tight">
