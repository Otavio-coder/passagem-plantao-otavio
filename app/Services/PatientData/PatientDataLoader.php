...
class PatientDataLoader
{
    ...
    public function get(): array
    {
        ...
        Log::info('[PatientDataLoader] sector='.$this->sectorId.' patients='.count($attendanceNumbers).' demo='.$tDemo.'ms loaders='.$tLoaders.'ms total='.$tTotal.'ms keys='.implode(',', $otherKeys));
    ...
    private function runLoaders(array $keys, array $attendanceNumbers): array
    {
        ...
        foreach ($keys as $key) {
            ...
            Log::info('[PatientDataLoader] key='.$key.' ms='.round((hrtime(true) - $t) / 1e6));
        }
